...
X = None
path = Path('saveValues/tfidfValues.txt')
...
if path.is_file() == False:
    ...
    X = pickle.load(open("saveValues/tfidfValues.txt", "rb"))
    tfidf_vector.words_list = pickle.load(open("saveValues/tfidfVector.txt", "rb"))
    review_actual_val = pickle.load(open("saveValues/reviewRatings.txt", "rb"))
...
corpus = []
review_actual_val = []
corpus, review_actual_val = test([TeP, TeN], stopwords, 0, int(int(sys.argv[1])/2))
X = tfidf_vector.transform(corpus)
